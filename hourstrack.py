...
class LedgerFlow:
    def __init__(self, root):
        ...
        self.use_tips = self.settings.get('use_tips', True)
        self.base_rate = float(self.settings.get('base_rate', 7.00))
        self.avg_tips = float(self.settings.get('avg_tips', 23.15)) if self.use_tips else 0.0
        self.effective_rate = float(self.settings.get('effective_rate', self.base_rate + (self.avg_tips if self.use_tips else 0)))
    ...
    def save_settings(self):
        ...
        self.settings['avg_tips'] = self.avg_tips
        self.settings['effective_rate'] = self.effective_rate
        
        with open(settings_path, 'w') as f:
            json.dump(self.settings, f, indent=2)

    def update_rates(self):
        """Recalculate effective rate based on current settings"""
        if self.use_tips:
            self.effective_rate = self.base_rate + self.avg_tips
        else:
    ...
    def show_settings(self):
        ...
        tips_check = ttk.Checkbutton(
            ...
            command=self.toggle_tips_settings
        )
        tips_check.pack(anchor=tk.W, pady=(0, 10))

        # Average tips (only shown if tips are enabled)
        ...
        self.update_effective_rate_display(settings_win)
        ...
        # Bind spinbox changes to update effective rate display
        self.base_rate_var.trace('w', lambda *args: self.update_effective_rate_display(settings_win))
        self.avg_tips_var.trace('w', lambda *args: self.update_effective_rate_display(settings_win))

    def toggle_tips_settings(self):
        """Show/hide tips input based on checkbox"""
        if self.use_tips_var.get():
            self.tips_spin_frame.pack(fill=tk.X, pady=(5, 0))
        else:
            self.tips_spin_frame.pack_forget()
        self.update_effective_rate_display(self.tips_spin_frame.winfo_toplevel())

    def update_effective_rate_display(self, parent_window):
        """Update the effective rate display in settings window"""
        try:
            base = float(self.base_rate_var.get())
            tips = float(self.avg_tips_var.get()) if self.use_tips_var.get() else 0
            effective = base + tips
            
            # Find or create the display label
            for widget in parent_window.winfo_children():
                if isinstance(widget, ttk.Frame):
                    for child in widget.winfo_children():
                        if hasattr(child, 'is_rate_display'):
                            child.destroy()
            
            # Create new display
            display_frame = ttk.Frame(parent_window.winfo_children()[0])  # main_frame
            display_frame.pack(fill=tk.X, pady=(10, 0))
            
            ttk.Label(
                display_frame,
                text=f"Effective Rate: ${effective:.2f}/hour",
                font=('Helvetica', 12, 'bold'),
                foreground='#2E7D32'
            ).pack()
            ttk.Label(
                display_frame,
                text=f"${base:.2f} base" + (f" + ${tips:.2f} tips" if self.use_tips_var.get() else ""),
                font=('Helvetica', 10),
                foreground='gray'
            ).pack()
            
            # Tag the label for easy removal
            for child in display_frame.winfo_children():
                child.is_rate_display = True
    ...
    def save_new_settings(self, settings_win):
        ...
        try:
            self.base_rate = float(self.base_rate_var.get())
            self.use_tips = self.use_tips_var.get()
            self.avg_tips = float(self.avg_tips_var.get()) if self.use_tips else 0.0
            
            if self.base_rate < 0 or self.avg_tips < 0:
    ...
    def update_rate_display(self):
        ...
        if self.use_tips:
            ...
            rate_text = f"Rate: ${self.effective_rate:.2f}/hr (no tips included)"
    ...
    def setup_ui(self):
        ...
        for h in range(1, 9):
            ttk.Button(
                ...
                text=f"{h}h",
                width=4,
                command=lambda v=h: self.hours_var.set(str(v))
        ...
        ttk.Button(input_frame, text="Add Entry", command=self.add_entry)\
        ...
        self.momentum_label = ttk.Label(
            left, text="", font=('Helvetica', 11, 'bold'), foreground='#2E7D32'
        )
        ...
        self.update_rate_display()  # Set initial text
        ...
        self.period_info_label = ttk.Label(
            right,
            text=f"Started: {self.current_period_start}",
            style='Rate.TLabel'
        )
        self.period_info_label.grid(row=1, column=0, sticky="w", pady=(5, 0))

        ttk.Button(right, text="Start New Period", command=self.start_new_period)\
            .grid(row=2, column=0, sticky="w", pady=(5, 0))

        ttk.Button(right, text="Export Data", command=self.export_data)\
            .grid(row=3, column=0, sticky="w", pady=(5, 0))

        ttk.Button(right, text="Clear All Data", command=self.clear_all_data)\
    ...
    def update_visualization(self, total_hours, entries):
        ...
        # 50% larger base (36) and modest scaling thereafter
        font_size = int(min(54, max(36, 36 + int(projected / 1000))))

        # Draw bars only if we have entries
        if entries:
            last = entries[:10]
            max_hours = max(r[1] for r in last)
            max_bar_h = h - 40
            # Keep scale stable; avoid divide-by-zero and tiny max
        ...
        # Optional minimal labels kept subtle
        self.canvas.create_text(
            w - 120, (h // 2) + 35,
    ...
    def clear_all_data(self):
        ...
        try:
            ...
            # IMPORTANT: keep UI/DB in sync in one place
            self.load_period_data()
            self.hours_entry.focus_set()
